...
import deepspeed
from utils import TASK_INST, SPECIAL_TOKENS
import mlflow
...
def encode_with_prompt_completion_format(example, tokenizer, max_seq_length):
    ...
    tokenized_example = tokenizer(source_text + target_text,
                                  return_tensors='pt',
    ...
    tokenized_prompt = tokenizer(source_text,
                                 return_tensors='pt',
                                 max_length=max_seq_length,
                                 padding='longest',
                                 truncation=True)
    ...
    return {
        "prompt": source_text,
        "target": target_text,
        "example": source_text + target_text,
...
def main():
    ...
    if isinstance(tokenizer, LlamaTokenizer) or isinstance(tokenizer, LlamaTokenizerFast):
        logger.info(f"llama2 special tokens: {tokenizer.special_tokens_map}")

        # add the special tokens to the tokenizer and model
        special_token_dict = {"additional_special_tokens": ["[No Answer]"]}
        ...
        assert num_added_tokens in [1, 2], "special tokens must be added to the original tokenizers."
    ...
    logger.info(f"number of added tokens: {num_added_tokens}")
    logger.info(f"tokenizer special tokens: {tokenizer.special_tokens_map}")
    logger.info(f"tokenizer total tokens: {len(tokenizer)}")
    ...
    # Record the prompt of training examples
    ...
    with accelerator.main_process_first():
        lm_datasets = raw_datasets.map(
            ...
            remove_columns=[name for name in raw_datasets["train"].column_names
                            if name not in ["prompt", "target", "example",
    ...
    train_dataset.map(
        lambda example: {col: example[col] for col in
                         ["prompt", "target", "example"]},
        batched=False,
        remove_columns=["input_ids", "labels", "attention_mask"],
        desc="Saving training prompts").to_json(f"{args.output_dir}/train_data_prompt.jsonl")
    train_dataset = train_dataset.remove_columns(["prompt", "target", "example"])
    ...
    if args.with_tracking:
        ...
        mlflow.log_artifacts(args.output_dir, "train_data_prompt.jsonl")
