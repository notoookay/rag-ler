...
def convert_cot_data(data_dir, output_dir, num_zero_shot_examples=50000, num_few_shot_examples=0):
    os.makedirs(output_dir, exist_ok=True)
    examples = []
...
def convert_flan_v2_data(data_dir, output_dir, data_file="tulu_v1_resampled_flan_100k.jsonl", num_samples=None):
    os.makedirs(output_dir, exist_ok=True)
    examples = []
    ...
    if num_samples:
        examples = random.sample(examples, k=num_samples)
    with open(output_path, "w") as fout:
        for idx, example in enumerate(examples):
            prompt = example["inputs"]
            if not prompt.endswith("\n") and not prompt.rstrip().endswith(":"):
...
def convert_oasst1_data(data_dir, output_dir, top_k_reply=None, num_examples=None):
    '''
    ...
    if num_examples:
        conversations = random.sample(conversations, k=num_examples)
    ...
    for dataset in args.dataset:
        if dataset == "tulu_v1":
            ...
            convert_cot_data(
                ...
                num_zero_shot_examples=COT_SIZE
            )
            convert_oasst1_data(
