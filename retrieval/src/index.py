...
class Indexer(object):
    ...
    def search_knn(self,
                   ...
                   top_docs: int,
                   use_gpu: bool = False,
                   index_batch_size: int = 2048) -> List[Tuple[List[object], List[float]]]:
        query_vectors = query_vectors.astype('float32')
        result = []
        nbatch = (len(query_vectors)-1) // index_batch_size + 1
        if use_gpu:
            print('Searching with GPU')
            ngpus = faiss.get_num_gpus()
            print(f'Number of GPUs: {ngpus}')
            clone_options = faiss.GpuMultipleClonerOptions()
            clone_options.shard = True
            gpu_index = faiss.index_cpu_to_all_gpus(self.index, co=clone_options)
        for k in tqdm(range(nbatch)):
            start_idx = k*index_batch_size
            end_idx = min((k+1)*index_batch_size, len(query_vectors))
            q = query_vectors[start_idx: end_idx]
            if use_gpu:
                scores, indexes = gpu_index.search(q, top_docs)
            else:
