...
class Retriever:
    ...
    def add_passages(self, data, passages, top_passages_and_scores):
    ...
    def search_document(self, data, query):
        questions_embedding = self.embed_queries(self.args, query)

        # get top k results
        start_time_retrieval = time.time()
        top_ids_and_scores = self.index.search_knn(questions_embedding, self.args.n_docs, self.args.use_faiss_gpu)
        print(f"Search time: {time.time()-start_time_retrieval:.1f} s.")

        self.add_passages(data, self.passage_id_map, top_ids_and_scores)
...
def main(args):
    ...
    for path in data_paths:
        ...
        retriever.search_document(data, queries)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--use_faiss_gpu", action="store_true", help="use faiss with gpu")
